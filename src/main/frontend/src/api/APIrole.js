...
const apirole = axios.create({
  baseURL: `${rurl}/role`, // our API base URL
});

// Request interceptor for adding the bearer token
apirole.interceptors.request.use(
  (config) => {
...
/* apirole.interceptors.response.use(
    ...
    async (error) => {
      ...
      if (error.response.status === 403 && !originalRequest._retry) {
        ...
        if (refreshtoken || !isTokenExpired(refreshtoken)) {
          ...
          return apirole(originalRequest);
        }
        else {
...
export const addRole = (formData) => {
  return apirole.post('', formData);
};

export const updateRole = (userId, formData) => {
  return apirole.put(`/${userId}`, (userId,formData));
};
