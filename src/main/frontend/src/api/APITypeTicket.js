...
const apitype_Ticket = axios.create({
  baseURL: `${rurl}/type_Ticket`, // our API base URL
});

// Request interceptor for adding the bearer token
apitype_Ticket.interceptors.request.use(
  (config) => {
...
/* apitype_Ticket.interceptors.response.use(
    ...
    async (error) => {
      ...
      if (error.response.status === 403 && !originalRequest._retry) {
        ...
        if (refreshtoken || !isTokenExpired(refreshtoken)) {
          ...
          return apitype_Ticket(originalRequest);
        }
        else {
...
// API endpoints
export const getType_Ticket = () => {
  return apitype_Ticket.get();
};

export const addType_Ticket = (formData) => {
  return apitype_Ticket.post('', formData);
};

export const updateType_Ticket = (userId, formData) => {
  return apitype_Ticket.put(`/${userId}`, (userId,formData));
};

// Export the api instance
export default apitype_Ticket;