...
const apidepartement = axios.create({
  baseURL: `${rurl}/departement`, // our API base URL
});

// Request interceptor for adding the bearer token
apidepartement.interceptors.request.use(
  (config) => {
...
/* apidepartement.interceptors.response.use(
    ...
    async (error) => {
      ...
      if (error.response.status === 403 && !originalRequest._retry) {
        ...
        if (refreshtoken || !isTokenExpired(refreshtoken)) {
          ...
          return apidepartement(originalRequest);
        }
        else {
...
export const getDepartement = () => {
  return apidepartement.get();
};

export const getDepartementBySocieteName = (name) => {
  return apidepartement.get(`/societe/${name}`, (name));
};


export const addDepartement = (formData) => {
  return apidepartement.post('', formData);
};

export const updateDepartement = (userId, formData) => {
  return apidepartement.put(`/${userId}`, (userId,formData));
};

// Export the api instance
export default apidepartement;