...
import TransfererDialog from "./TransfertDialog";

export default function SuivreMesResolutions() {
  ...
  const fetchDataGetSuiviPersonnel = async () => {
    try {
      const response = await getSuivi_Ticket_Superviseur_en_cours_resolution(username);
      setListSuiviPersonnel(response.data);
    } catch (error) {
      console.error('Error fetching list personnel by encoursresolution:', error);
    }
  };
  ...
  const Displayresolution = (row) => {
    setSelectedSuivi(() => row);
    setOpenResolution(true);
  };


  const Displaytransfert = (row) => {
    setSelectedSuivi(() => row);
  ...
  return (
    <Grid container spacing={2}>
      ...
      <Grid size={12}>
        <MainCard title="Liste tickets à suivre">
          <TextField
      ...
      <ResolutionDialog open={openResolution} setOpen={setOpenResolution} data={selectedSuivi} isConfirm={isConfirm}
        setIsConfirm={setIsConfirm} setMessage={setMessage} setSeverity={setSeverity} setOpenMessage={setOpenMessage} setColor={setColor} />
      <DisplayDialog open={openDisplay} setOpen={setOpenDislplay} data={selectedSuivi} />

      <TransfererDialog
        openTransfert={openTransfert}
        setIsTransfert={setIsTransfert}
        setOpenTransfert={setOpenTransfert}
        data={selectedSuivi}
        handleCloseDialogTransfert={handleCloseDialogTransfert}
        isConfirm={isConfirm}
