...
import { getAllListPersonnelByNomSociete, getListPersonnelByNomSociete } from "../../api/APIpersonnel";
import { useEffect, useState } from "react";

export default function TransfertUserTable({ data, selected, setSelected, assignateur }) {

  const [listPersonnel, setListPersonnel] = useState([]);

  const fetchDataGetPersonnelFromSameSociete = async () => {
    try {
      const response = await getAllListPersonnelByNomSociete(assignateur);
      setListPersonnel(response.data);
    } catch (error) {
      console.error('Error fetching list personnel by assignateur:', error);
    }
  };
